...
PPMI_te = patients_cov.index[patients_cov['site_PPMI'] == 1].to_list()
...
patients_cov = patients_cov.drop(['subjectID', 'site_ID_bin', 'site_PPMI'], axis=1)
...
hbr_site_metrics.to_csv(os.path.join(data_dir, 'hbr_Parkinsons_site_metrics_f1.csv'), index=False, index_label=None)

# Save overall test set metrics to CSV file
hbr_metrics.to_csv(os.path.join(data_dir, 'hbr_Parkinsons_metrics_f1.csv'), index=False, index_label=None)
