...
parser.add_argument('-roi','--roi', help='Name of the ROI to analyze. Just one.',required=True)
...
print("Name of the ROI to process: %s" % args.roi)
...
X_train8020_f1, X_test8020_f1, y_train8020_f1, y_test8020_f1 = train_test_split(controls_cov_big, 
                                                                                ...
                                                                                test_size=0.2, random_state=20018)
...
for roi in roi_ids: 
    ...
    batch_tr = os.path.join(data_dir, 'batch_tr.txt')
    batch_te = os.path.join(data_dir, 'batch_te.txt')
...
hbr_site_metrics.to_csv(''.join([data_dir, '/', 'hbr_controls_site_metrics_f1_', args.roi, '.csv']), index=False, index_label=None)

# Save overall test set metrics to CSV file
hbr_metrics.to_csv(''.join([data_dir, '/', 'hbr_controls_metrics_f1_', args.roi, '.csv']), index=False, index_label=None)
