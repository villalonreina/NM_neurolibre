...
NIMHANS_0_te = controls_cov.index[controls_cov['site_NIMHANS_0'] == 1].to_list()
NIMHANS_1_te = controls_cov.index[controls_cov['site_NIMHANS_1'] == 1].to_list()
...
dfbatch_adapt_tr = controls_cov[['site_ID_bin']]
...
controls_cov = controls_cov.drop(['subjectID', 'site_ID_bin', 'site_NIMHANS_0', 'site_NIMHANS_1'], axis=1)
...
batch_effects_test_txfr = patients[['SID']]
X_test_txfr = patients[[args.age_column, args.sex_column]]
...
for roi in roi_ids: 
    ...
    # load test response files (patients).
    testrespfile_path = os.path.join(roi_dir, 'resp_adapt_patients_te.pkl')
    print(os.path.join(roi_dir, 'resp_adapt_patients_te.pkl'))
    ...
    # load true test data
    X_te = np.loadtxt(covfile)
    y_te = np.loadtxt(respfile)
