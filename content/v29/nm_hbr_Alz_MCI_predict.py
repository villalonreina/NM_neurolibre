...
parser = argparse.ArgumentParser(description='This run the hbr normative modeling with HBR tp predict the Z-scores from a newly given dataset.')
...
roi_ids = ['ACR', 'ALIC', 'Average', 'BCC', 'CGC', 'CGH', 'CST', 'EC', 'FX', 'FXST', 'GCC', 'UNC', 'PCR', 'PLIC', 'PTR', 'RLIC', 'SCC', 'SCR', 'SFO', 'SLF', 'SS', 'TAP']
...
patients_cov['site_ID_bin'] = patients['Protocol_No']  # this is the numeric version of Protocol
...
ADNI3_GE36_te = patients_cov.index[patients_cov['site_ADNI3_GE36'] == 1].to_list()
ADNI3_GE54_te = patients_cov.index[patients_cov['site_ADNI3_GE54'] == 1].to_list()
ADNI3_P33_te = patients_cov.index[patients_cov['site_ADNI3_P33'] == 1].to_list()
ADNI3_P36_te = patients_cov.index[patients_cov['site_ADNI3_P36'] == 1].to_list()
ADNI3_S127_te = patients_cov.index[patients_cov['site_ADNI3_S127'] == 1].to_list()
ADNI3_S31_te = patients_cov.index[patients_cov['site_ADNI3_S31'] == 1].to_list()
ADNI3_S55_te = patients_cov.index[patients_cov['site_ADNI3_S55'] == 1].to_list()
OASIS3_te = patients_cov.index[patients_cov['site_OASIS3'] == 1].to_list()

# Creating batch/site file
dfbatch_te = patients_cov[['site_ID_bin']]
...
patients_cov = patients_cov.drop(['subjectID', 'site_ID_bin', 'site_ADNI3_GE36', 'site_ADNI3_GE54', 'site_ADNI3_P33', 'site_ADNI3_P36', 'site_ADNI3_S127', 'site_ADNI3_S31', 'site_ADNI3_S55', 'site_OASIS3'], axis=1)
patients_cov.loc[:, args.age_column] = patients_cov[args.age_column] / 100

sites = [ADNI3_GE36_te, ADNI3_GE54_te, ADNI3_P33_te, ADNI3_P36_te, ADNI3_S127_te, ADNI3_S31_te, ADNI3_S55_te, OASIS3_te]
site_names = ['ADNI3_GE36_te', 'ADNI3_GE54_te', 'ADNI3_P33_te', 'ADNI3_P36_te', 'ADNI3_S127_te', 'ADNI3_S31_te', 'ADNI3_S55_te', 'OASIS3_te']
...
for roi in roi_ids: 
    ...
    np.savetxt(os.path.join(roi_dir, 'cov_int_patients_te.txt'), patients_cov)
    
    # Saving the Y for each roi
    np.savetxt(os.path.join(roi_dir, 'resp_patients_te.txt'), patients[roi])
...
for roi in roi_ids: 
    ...
    cov_file_te = os.path.join(roi_dir, 'cov_int_patients_te.txt')
    print(os.path.join(roi_dir, 'cov_int_patients_te.txt'))
    
    # load test response files
    resp_file_te = os.path.join(roi_dir, 'resp_patients_te.txt')
    print(os.path.join(roi_dir, 'resp_patients_te.txt'))
...
hbr_site_metrics.to_csv(os.path.join(data_dir, 'hbr_Alzheimers_site_metrics_f1.csv'), index=False, index_label=None)

# Save overall test set metrics to CSV file
hbr_metrics.to_csv(os.path.join(data_dir, 'hbr_Alzheimers_metrics_f1.csv'), index=False, index_label=None)
