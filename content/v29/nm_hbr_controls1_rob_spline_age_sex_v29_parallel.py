...
X_train8020_f1, X_test8020_f1, y_train8020_f1, y_test8020_f1 = train_test_split(controls_cov_big, 
                                                                                ...
                                                                                test_size=0.2, random_state=63289)
...
for roi in roi_ids: 
    ...
    python_path = '/home/preclineu/seykia/.conda/envs/python3.8/bin/python'
    ...
    ptk.normative_parallel.execute_nm(processing_dir, python_path,
                   'Estimate_' + job_name, covfile, respfile, batch_size, memory, duration, alg='hbr', 
                   log_path=log_dir, binary=True, testcovfile_path = testcovfile_path, 
                   testrespfile_path = testrespfile_path, inscaler=inscaler, outscaler=outscaler,
                   ...
                   savemodel='True', saveoutput='True', outputsuffix='_estimate',
                   n_samples='1000', interactive='auto')
    
    collect_nm(processing_dir,
                   job_name='Estimate_' + job_name,
                   collect=False,
                   batch_size=batch_size,
                   outputsuffix='_estimate')
    
    # If there are failed jobs on the first collect:nm run then we have to run rerun_nm
    rerun_nm(processing_dir,
                 log_path,
                 memory,
                 duration,
                 binary=False):
    ...
    # If the number of jobs is Zero with collect=False then you run the collect=True.
    # It will merge collect all results from parallel
    collect_nm(processing_dir,
                   job_name='Estimate_' + job_name,
                   collect=True,
                   batch_size=batch_size,
