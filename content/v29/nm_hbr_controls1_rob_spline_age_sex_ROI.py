# -*- coding: utf-8 -*-
"""
Created on Sat Mar 16 23:08:07 2024
...
parser.add_argument('-roi','--roi', type=str, nargs='+', help='List of ROIS. One or more. Strings separated by spaces',required=True)
...
hbr_site_metrics.to_csv(''.join([data_dir, '/', 'hbr_controls_site_metrics_f1_', '_'.join(args.roi), '.csv']), index=False, index_label=None)

# Save overall test set metrics to CSV file
hbr_metrics.to_csv(''.join([data_dir, '/', 'hbr_controls_metrics_f1_', '_'.join(args.roi), '.csv']), index=False, index_label=None)
